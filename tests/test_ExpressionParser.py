...
def test_clean_expression():
    _._clean_expression("32 - (3 / 45) * (4( + (3 - 45)) *   ( 8 / 4 )") == "32-(3/45)*(4+(3-45))*(8/4)"
    _._clean_expression("2 + (3/ (3* (3- 3- ( 32))))") == "2+(3/(3*(3-3-(32))))"

def test_check_unknown_simbols():
    with raises(UnknownSimbol): _._check_unknown_simbols("32 - (3 * d) / s")

def test_parse_expression():
    assert _._parse_expression("3+(3/5*(234-1+1.12))") == ['(',D(3),'+','(',D(3),'/',D(5),'*','(',D(234),'-',D(1),'+',D('1.12'),')',')',')']

def test_is_parenthesis():
    assert _._is_parenthesis(')') == True
    assert _._is_parenthesis('(') == True
    assert _._is_parenthesis('()') == False
    assert _._is_parenthesis('*') == False
